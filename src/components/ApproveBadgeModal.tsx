...
interface ApproveBadgeModalProps {
  open: boolean;
  onClose: () => void;
  assigneeName: string;
}

const ApproveBadgeModal: React.FC<ApproveBadgeModalProps> = ({ open, onClose, assigneeName }) => {
  useEffect(() => {
    if (open) {
  ...
  return (
    ...
    >
      <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center' }}>
        ...
        <Typography variant="body1" sx={{ fontWeight: 600, fontSize: '1.1rem' }}>
          Badge Approved! {assigneeName} has been notified.
        </Typography>
      </DialogContent>
    </Dialog>
  );
...
export default ApproveBadgeModal;